...
* 변수 이름 규칙
    ...
    * 대소문자를 구분한다.
    * class와 생성자함수는 첫 문자를 대문자로, 나머지는 보통 소문자로 시작한다.
* var, let, const
    ```js
    if (true) {
...
* nullish 병합 연산자(??)
    ...
    ```
* typeof 연산자
* 프로퍼티(멤버) 접근 연산자, 오브젝트접근연산자
    let test={one:1}
    1. 마침표 프로퍼티 접근 연산자 test.one
    2. 대괄호 프로퍼티 접근 연산자 test['one']
    10.toString() 소수점인식해서 오류
* 관계 연산자
    * 키만 가지고 판단 0 1 2
    ```js
    ...
    1 in [10, 20, 30] // true
    1 in 'hello' // error
    h in 'hello' // error 키와 벨류를 가지고 판단
    'name' in {'name':'hojun', 'age':10} //true
    'length' in [10, 20, 30]; // true 배열에 length값이 있음 
    ```
...
- Number(숫자)
    ...
    * 메서드 : 
        ...
        * num.toFixed() 고정 소수점 표기법을 사용하여 숫자 형식을 지정
    * Number()
    ...
    * Math
        ...
        * Math.floor()
        * Math.round()
        * Math.random() 0부터 10까지 원하는 숫자 랜덤 Math.floor(Math.random()*11)
        * Math.abs()
        * Math.sqrt() **0.5 = 0.5승 = 루트
        * Math.pow()
    * NaN
    * Infinity, -Infinity
...
- String(문자열)
    ...
    * 메서드 : 
        ...
        * str.concat()
        * str.repeat()
        * str.splice() -> 업데이트 사항 : 배열에 있는 메서드 str에는 없다 
        ```js
        'hello'.repeat(100)
        '0'.repeat(100)
        '5'.repeat(100).split('').map(Number)
        '5'.repeat(10).split('').map(parseInt)에서 NaN이 출력되는 이유
...
- Array(배열) : object
    ...
    * 메서드 : 
        * length
        * forEach 단순 순회
        * map 데이터를 맵핑해서 뽑아내는용도 새로운 어레이를 만들어냄 리턴된값 언디파인드
        * filter 어떤 데이터에 범주에 속하는 것들을 뽑아내기위해
        * push / pop - mutable(원본수정가능)
        * slice - immutable(원본수정이 불가능) 잘라내는것
        * splice - mutable 교체하는것
        * reduce - immutable 누적하는것
        * join 합치는것
        * indexOf 인덱스 찾아내는것
        * includes 인덱스 포함하는지 찾아내는것
        * find 찾아내는것 
        * concat  이어붙이는것 
        * every
        * some
...
- 함수 표현식과 함수 선언식
    ```js
    let 함수표현식 = function(){} // 호이스팅 X 안되는것처럼 보인다 TDG
...
let a = 0b1001 // a == 9
let b = 0o1001 // b == 513
let num = [010, 1424, 4555] num[0]찍으면 8 나옴 앞에 0붙으면 8진수로 처리 문자열로 바꿔서 처리해야함
let c = 0x1001 // c == 4097
```
